...
function GalleryControls() {
  ...
  return (
    <>
      <button
        type="button"
        className="absolute bottom-[7%] left-4 z-10 inline-flex h-[96px] w-[72px] items-center justify-center text-white transition hover:scale-105 disabled:pointer-events-none disabled:opacity-30 sm:left-7"
      ...
      <button
        type="button"
        className="absolute bottom-[7%] right-4 z-10 inline-flex h-[96px] w-[72px] items-center justify-center text-white transition hover:scale-105 disabled:pointer-events-none disabled:opacity-30 sm:right-7"
      ...
      <CarouselContent className="-ml-0">
        {items.map((item) => (
          <CarouselItem key={item.id} className="pl-0">
            <article className="relative aspect-[1520/750] min-h-[420px] overflow-hidden bg-[#212322] text-white">
              <Image
                src={item.imageUrl}
                alt={item.imageAlt}
                fill
              ...
              <div className="absolute inset-x-0 bottom-[6%] px-[13%]">
                ...
                {item.description ? (
                  <p className="mt-3 max-w-[1123px] text-[clamp(1rem,2vw,1.571rem)] font-light leading-normal text-white">
                    {item.description}
                  </p>
                ) : null}
              </div>
