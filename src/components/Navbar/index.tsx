...
export function Navbar({
  ...
  return (
    <header
      className={cn(
        'z-30 w-full  px-5 py-6 sm:px-8 lg:px-12',
        hasBackgroundImage ? 'absolute left-0 top-0' : 'relative',
        className,
      )}
    >
      <nav
        className={cn(
          'mx-auto flex layout-container items-center justify-between rounded-md px-5 py-4 transition-colors duration-200 sm:px-8 lg:px-[50px]',
          hasBackgroundImage
            ? 'bg-transparent text-white'
            : 'bg-[#151817] text-white shadow-[0_10px_28px_rgba(0,0,0,0.2)]',
        )}
