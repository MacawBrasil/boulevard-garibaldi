...
const galleryDateFormatter = new Intl.DateTimeFormat('pt-BR', {
  day: '2-digit',
  month: '2-digit',
  year: 'numeric',
  timeZone: 'America/Sao_Paulo',
...
function toGalleryCarouselItem(item: Gallery): GalleryCarouselItem | null {
  const image = typeof item.image === 'string' ? null : item.image
  const imageUrl = getMediaURL(image)
  ...
  return {
    ...
    title: item.title || '',
    description: item.description || '',
    dateLabel: item.date ? galleryDateFormatter.format(new Date(item.date)) : '',
    imageUrl,
    imageAlt: image?.alt || item.title || 'Galeria de eventos',
  }
}
...
export default async function EventsPage() {
  ...
  const galleryItems = galleryResult.docs.map(toGalleryCarouselItem).filter((item) => item !== null)
    ...
    <>
      ...
      <main className="relative overflow-hidden bg-[#f4f4f4] text-[#212322]">
        ...
        <section className="relative pb-16 pt-6">
          <div className="layout-container">
            ...
            <div className="mt-10 text-center">
              <h1 className="text-[clamp(3rem,7vw,4.625rem)] font-bold leading-normal tracking-normal">
                {page.title}
              </h1>
            ...
            {events.length ? (
              ...
              <p className="mt-12 text-center text-lg text-[#212322]/70">
        ...
        <section className="relative pb-20">
          <div className="layout-container">
            <div className="mb-12 grid items-start gap-6 md:grid-cols-2">
              <h2 className="text-[clamp(3rem,6vw,3.875rem)] font-bold leading-normal tracking-normal">
                Galeria
              </h2>
              <p className="max-w-[445px] text-left text-[clamp(1.375rem,3vw,2.25rem)] font-normal leading-normal md:ml-auto md:text-right">
                Confira alguns de nossos eventos passados
              </p>
            </div>

            {galleryItems.length ? (
              <EventsGalleryCarousel items={galleryItems} />
            ) : (
              <p className="text-lg text-[#212322]/70">Nenhuma imagem cadastrada na galeria.</p>
            )}
          </div>
        </section>
